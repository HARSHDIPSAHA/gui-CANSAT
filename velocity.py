...
class RealTimePlot:
    def __init__(self, root, dataframe):
        ...
        self.ax.set_ylim(dataframe['velocity'].min() - 5, dataframe['velocity'].max() + 5)
    ...
    def update_plot(self, frame):
        if self.current_index < len(self.data):
            self.line.set_data(self.data['time'][:self.current_index], self.data['velocity'][:self.current_index])
            self.ax.relim()
